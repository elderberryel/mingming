...
(function () {
    ...
    const hideSelectors = [
        ...
        'div[style*="bkcdn.net"]',
        // 广告 ins 标签及 centbtd 容器隐藏
        '.centbtd',
        'ins.eas6a97888e2',
        'ins[data-zoneid="5781030"]',
        'ins[data-processed="true"]',
        // 动态前缀视频广告及外层容器隐藏 (mFyRACzL 等)
        '.wb-contai',
    ...
    function parseAdblockLine(line, category, label) {
        ...
        if (idx >= 0) {
            ...
            if (!left) {
                return {
                    cssRules: [{ domain: '*', selector, category, listName: label }]
                };
            }
        ...
        if (hosts.length) {
            return {
                domains: hosts.map(domain => ({ domain, category, listName: label }))
            };
        }
        ...
        if (s.startsWith('||')) {
            ...
            if (x.includes('.') && /^[a-z0-9.-]+$/i.test(x)) {
                return {
                    domains: [{ domain: x, category, listName: label }]
                };
            }
        }
        ...
        if (/^[a-z0-9.-]+\.[a-z]{2,}$/i.test(s)) {
            return {
                domains: [{ domain: s.toLowerCase().replace(/^www\./, ''), category, listName: label }]
            };
        }
    ...
    function googleCleanSearch() {
        ...
        function hideAI() {
            ...
            try {
                document.querySelectorAll('#eKIzJc').forEach(el => {
                    el.style.setProperty('display', 'none', 'important');
                });
            } catch (_) {}
        }
    ...
    async function cleanSearchMain() {
        registerMenus();
        googleCleanSearch();
        watchSpa();
