...
(function() {
    ...
    const cssRules = hideSelectors.map(sel => `${sel} { display: none !important; }`).join('\n');
    GM_addStyle(cssRules);

    function removeOrHide(selector) {
        try {
    ...
    function removeTrackingPixels() {
        const selectors = ['img', 'iframe', 'script', 'link', 'object', 'embed'];
        const elements = document.querySelectorAll(selectors.join(','));
        elements.forEach(el => {
            let src = '';
            if (el.src) src = el.src;
            else if (el.href) src = el.href;
            else if (el.data) src = el.data;
    ...
    function applyAllRules() {
        hideSelectors.forEach(sel => removeOrHide(sel));
        removeTrackingPixels(); // 移除追踪像素
    }

    function hideLuckyButton() {
    ...
    async function init() {
        ...
        const start = () => {
            ...
            window._observer = new MutationObserver(() => {
                ...
                window._hideDebounce = setTimeout(() => {
                    applyAllRules();
                    hideLuckyButton();
                }, 150);
            });
