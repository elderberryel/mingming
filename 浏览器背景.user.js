...
if(SiteAdapters.isXSite()){
  ...
  // X 媒体兜底：图片/视频/媒体容器保持不透明，禁止被全局清场
  css+=`
...
_inject(root){
 ...
 for(let i=0;i<nodes.length;i++){
  ...
  if(!sr||ShadowFixer._done.has(sr))continue;
  ShadowFixer._done.add(sr);
  if(ShadowFixer._skipHost(el))continue;
  try{
