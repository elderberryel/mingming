...
(function () {
    ...
    const CAPTCHA_SELECTORS = ['#challenge-running', '#challenge-stage', '#cf-challenge-running', '#cf-wrapper', '#cf-section', '.cf-turnstile', '#turnstile-wrapper', 'iframe[src*="challenges.cloudflare.com"]', 'iframe[src*="hcaptcha.com"]', 'iframe[src*="recaptcha.net"]', '.h-captcha', '.g-recaptcha', '[id^="cf-"]', '[id^="challenge-"]'];
...
*:not(img):not(svg):not(video):not(canvas):not(.translate-ui):not(.translate-ui *):not([id^="cf-"]):not([id^="challenge-"]):not(.cf-turnstile):not(.h-captcha):not(.g-recaptcha){background-color:transparent!important;}
...
#challenge-running, #challenge-stage, #cf-challenge-running, .cf-turnstile, #turnstile-wrapper, #cf-wrapper, iframe[src*="challenges.cloudflare.com"], iframe[src*="hcaptcha.com"], iframe[src*="recaptcha.net"], .h-captcha, .g-recaptcha, [id^="cf-"], [id^="challenge-"], #challenge-running *, #challenge-stage *, #cf-challenge-running *, .cf-turnstile *, #turnstile-wrapper *, #cf-wrapper *, .h-captcha *, .g-recaptcha * { background-color: initial !important; background: initial !important; backdrop-filter: none !important; -webkit-backdrop-filter: none !important; filter: none !important; opacity: 1 !important; pointer-events: auto !important; mix-blend-mode: normal !important; }
...
td,th,thead,tbody,tfoot,.rounded-top-2,.rounded-bottom-2,.rounded-2{background:transparent!important;background-color:transparent!important;}`;
    ...
    function getNativeBlurCSS(blurAmount) { if (blurAmount <= 0) return ''; return `.modal:not([id^="cf-"]):not([id^="challenge-"]),.dialog,.popup,.dropdown,.menu,.popover,.tooltip,[role="dialog"],[role="menu"],[role="tooltip"],.layer,.fancybox,.swal-modal,.ant-modal,.el-dialog,.el-popper,.notification,.Toastify__toast-container,.position-fixed,.z-50{backdrop-filter:blur(${blurAmount}px)!important;-webkit-backdrop-filter:blur(${blurAmount}px)!important;}`; }
    function buildCSS(cfg, finalUrl) { return getBackgroundCSS(cfg, finalUrl) + getThemeCSS(cfg.theme); }

    function isExcludedElement(el) { if (!el || !el.nodeType) return false; if (el.id === 'goTopBottom' || el.id === 'tbSettingsBtn' || el.id === 'tbSettingsPanel') return true; if (el.classList && (el.classList.contains('tb-settings-btn') || el.classList.contains('tb-settings-panel'))) return true; if (el.classList && el.classList.contains('translate-ui')) return true; if (el.id === 'tuPanel' || el.id === 'tuBtn') return true; if (el.tagName === 'IFRAME') { const src = el.src || ''; if (src.includes('challenges.cloudflare.com') || src.includes('hcaptcha.com') || src.includes('recaptcha')) return true; } const id = el.id || '', cls = (el.className && typeof el.className === 'string') ? el.className : ''; if (id === 'challenge-running' || id === 'challenge-stage' || id === 'cf-challenge-running' || id === 'cf-wrapper' || id === 'turnstile-wrapper' || id.startsWith('cf-') || id.startsWith('challenge-')) return true; if (cls.includes('cf-turnstile') || cls.includes('h-captcha') || cls.includes('g-recaptcha')) return true; try { if (el.closest && el.closest('#challenge-running, #challenge-stage, .cf-turnstile, #turnstile-wrapper, .h-captcha, .g-recaptcha, #cf-wrapper')) return true; } catch(e) {} return false; }
    ...
    function checkCaptchaStatus() { const title = (document.title || '').toLowerCase(); if (/just a moment|attention required|cloudflare|please wait|checking your browser|verify you are human/.test(title)) { if (!captchaActive) { captchaActive = true; removeStyle(); } return; } let found = false; for (const sel of CAPTCHA_SELECTORS) { try { if (document.querySelector(sel)) { found = true; break; } } catch(e) {} } if (found && !captchaActive) { captchaActive = true; removeStyle(); } else if (!found && captchaActive) { captchaActive = false; applyStyle(); } }
    ...
    // ★ 核心修复：彻底重写导出逻辑，使用 arrayBuffer 读取缓存，并增加本地图失败精准提示
            ...
            if (isCacheKey(globalUrl)) { 
                ...
                if (CACHE_AVAILABLE) { 
                    try { 
                        ...
                        if (response) globalRemoteData = new Uint8Array(await response.arrayBuffer()); // 改用 arrayBuffer
                ...
                if (isCacheKey(url)) { 
                    ...
                    if (CACHE_AVAILABLE) { 
                        try { 
                            ...
                            if (response) imgData = new Uint8Array(await response.arrayBuffer()); // 改用 arrayBuffer
