...
(function () {
    ...
    console.log('[浏览器背景] Cache API 可用:', CACHE_AVAILABLE);
    ...
    function getFilenameMap() {
        return safeJSONParse(getValue(GM_FILENAME_MAP, '{}'), {});
    }

    function setFilenameMap(map) {
        setValue(GM_FILENAME_MAP, JSON.stringify(map));
    }
    ...
    function isCacheKey(url) {
        return typeof url === 'string' && url.startsWith(CACHE_PREFIX);
    }
    ...
    function extractCacheKey(url) {
        if (!isCacheKey(url)) return null;
        return url.replace(CACHE_PREFIX, '');
    }

    function buildCacheKeyUrl(key) {
    ...
    async function storeImage(imageDataUrl, key, originalFilename) {
        ...
        if (CACHE_AVAILABLE) {
            try {
                ...
                console.log('[浏览器背景] 图片已存入 Cache API:', cacheKey);
            } catch (e) {
                console.warn('[浏览器背景] Cache API 存储失败，回退到 GM:', e.message);
            }
        }
        ...
        if (originalFilename) {
            saveOriginalFilename(cacheKey, originalFilename);
        }
    ...
    function saveGMBackup(key, dataUrl) {
        try {
            ...
            console.error('[浏览器背景] GM 备份保存失败:', e);
            alert('图片存储失败：GM 存储空间可能已满');
        }
    }
    ...
    async function getImage(key) {
        if (CACHE_AVAILABLE) {
            try {
                ...
                console.warn('[浏览器背景] Cache API 读取失败:', e.message);
            }
        }
    ...
    async function restoreToCache(key) {
        if (!CACHE_AVAILABLE) return false;
        
        const backup = getValue(GM_BACKUP_PREFIX + key, '');
        if (!backup || !backup.startsWith('data:image/')) return false;
    ...
    async function deleteImage(key) {
        ...
        try {
            GM_deleteValue(GM_BACKUP_PREFIX + key);
        } catch (e) {}
...
*:not(img):not(svg):not(video):not(canvas){background-color:transparent!important;}
...
td,th,thead,tbody,tfoot,.rounded-top-2,.rounded-bottom-2,.rounded-2{background:transparent!important;background-color:transparent!important;}`;
    ...
    function buildCSS(cfg, finalUrl) {
        return getBackgroundCSS(cfg, finalUrl) + getThemeCSS(cfg.theme);
    }
    ...
    async function preloadImage() {
        ...
        if (isCacheKey(cfg.url)) {
            const cacheKey = extractCacheKey(cfg.url);
            const imgUrl = await getImage(cacheKey);
            if (imgUrl) {
                _currentImageUrl = imgUrl;
            } else {
                console.warn('[浏览器背景] 图片未找到:', cacheKey);
                _currentImageUrl = null;
            }
        } else if (isDataImageUrl(cfg.url)) {
            console.log('[浏览器背景] 迁移旧版 Base64 数据...');
            const cacheKey = await storeImage(cfg.url, null, 'migrated_image.jpg');
            if (cacheKey) {
                const newUrl = buildCacheKeyUrl(cacheKey);
                if (getSiteConfig(getHost()) && getSiteConfig(getHost()).url === cfg.url) {
                    setSiteConfig(getHost(), Object.assign({}, getSiteConfig(getHost()), { url: newUrl }));
                } else {
                    setValue('Vie背景图片', newUrl);
                }
                configCache = null;
                _currentImageUrl = cfg.url;
            }
        } else {
            _currentImageUrl = cfg.url;
        }
    ...
    function applyStyle() {
        ...
        const finalUrl = _currentImageUrl || '';

        const s = ensureStyleNode();
        s.textContent = buildCSS(cfg, finalUrl);
        applyNativeBlur(cfg.nativeElementBlur);
    }

    async function applyStyleFull() {
        if (!_imageReady) {
            await preloadImage();
        }
        applyStyle();
    ...
    function applyAgain() {
        ...
        applyStyleFull();
        requestOverlayApply();
        setTimeout(applyStyleFull, 100);
        setTimeout(applyStyleFull, 500);
    }
            ...
            reader.onload = async function () {
                ...
                if (!isWebP && result.length > COMPRESS_THRESHOLD) {
                    const compressed = await new Promise(resolve => {
                        compressImage(result, COMPRESS_TARGET, resolve);
                    });
                    if (compressed) {
                        const originalKB = Math.round(result.length / 1024);
                        const compressedKB = Math.round(compressed.length / 1024);
                        const reduction = Math.round((1 - compressed.length / result.length) * 100);
                ...
                if (cacheKey) {
                    const cacheKeyUrl = buildCacheKeyUrl(cacheKey);
                    const sizeKB = Math.round(dataToStore.length / 1024);
                    console.log('[浏览器背景] 图片已存储:', cacheKey, `(${sizeKB}KB)`, '原始文件名:', originalFilename);
                    callback(cacheKeyUrl, file, cacheKey);
                } else {
                    alert('图片存储失败，请重试');
                }
            };
    ...
    function crc32(u8) {
        let crc = 0xFFFFFFFF;
        for (let i = 0; i < u8.length; i++) crc = CRC32_TABLE[(crc ^ u8[i]) & 0xFF] ^ (crc >>> 8);
    ...
    async function compressSmart(data) {
        ...
        try {
            ...
            if (deflated.length < data.length) return { compressed: deflated, method: 8 };
            return { compressed: data, method: 0 };
        } catch (e) {
            return { compressed: data, method: 0 };
        }
    }
    ...
    async function buildZip(entries) {
        ...
        for (const p of processed) {
            ...
            writeU16(view, offset + 8, p.method);
            writeU16(view, offset + 10, 0);
            writeU16(view, offset + 12, 0);
            writeU32(view, offset + 14, p.crc);
            writeU32(view, offset + 18, p.compressedSize);
            writeU32(view, offset + 22, p.originalSize);
            writeU16(view, offset + 26, p.nameBytes.length);
            writeU16(view, offset + 28, 0);
            u8.set(p.nameBytes, offset + 30);
            u8.set(p.compressed, offset + 30 + p.nameBytes.length);
            offset += 30 + p.nameBytes.length + p.compressedSize;
        }
        ...
        for (let i = 0; i < processed.length; i++) {
            ...
            writeU16(view, offset + 10, p.method);
            writeU16(view, offset + 12, 0);
            writeU16(view, offset + 14, 0);
            writeU32(view, offset + 16, p.crc);
            writeU32(view, offset + 20, p.compressedSize);
            writeU32(view, offset + 24, p.originalSize);
        ...
        writeU16(view, offset + 6, 0);
        writeU16(view, offset + 8, processed.length);
        writeU16(view, offset + 10, processed.length);
        writeU32(view, offset + 12, centralDirSize);
        writeU32(view, offset + 16, centralDirStart);
    ...
    async function parseZipForImport(zipData) {
        ...
        while (pos + 30 <= zipData.length) {
            ...
            const nameBytes = zipData.slice(pos + 30, pos + 30 + nameLen);
    ...
    function getExtFromFilename(filename) {
        ...
        return parts.pop().toLowerCase().replace('jpeg', 'jpg');
    }
        ...
        fetch(url, { mode: 'cors', credentials: 'omit', signal: controller.signal })
            .then(resp => {
                ...
                return resp.arrayBuffer().then(ab => {
                    ...
                    const ext = contentType.split(';')[0].trim().split('/').pop() || 'jpeg';
                    resolve({ data: new Uint8Array(ab), ext });
                });
            })
            .catch(err => {
                clearTimeout(timer);
                reject(new Error('下载图片失败: ' + (err.message || err)));
            });
    }
        ...
        return new Promise((resolve, reject) => {
            if (typeof GM_xmlhttpRequest === 'function') {
                ...
                const timer = setTimeout(() => {
                    if (!settled) { settled = true; fetchFallback(url, resolve, reject); }
                }, 15000);
                            ...
                            if (res.status >= 200 && res.status < 300) {
                                try {
                                    ...
                                    if (respData instanceof ArrayBuffer) {
                                        const headerStr = String(res.responseHeaders || '');
                                        ...
                                        fetchFallback(url, resolve, reject);
                                    }
    ...
    async function exportAllConfigZip() {
        try {
            ...
            if (isCacheKey(globalUrl)) {
                const cacheKey = extractCacheKey(globalUrl);
                const originalName = getOriginalFilename(cacheKey);
                const ext = originalName ? getExtFromFilename(originalName) : 'jpg';
                ...
                if (CACHE_AVAILABLE) {
                    try {
                        ...
                        if (response) {
                            const blob = await response.blob();
                            globalRemoteData = new Uint8Array(await blob.arrayBuffer());
                        }
                    } catch (e) {}
                }
                if (!globalRemoteData) {
                    ...
                    if (backup && backup.startsWith('data:image/')) {
                        globalRemoteData = dataUrlToUint8Array(backup);
                    }
                }
                ...
                globalRemoteUrl = globalUrl;
                try {
                    const { data, ext } = await fetchImageAsUint8(globalUrl);
            ...
            const imageEntries = [];
            if (globalRemoteData && globalImageFile) {
                imageEntries.push({ name: 'images/' + globalImageFile, data: globalRemoteData });
            }
            ...
            for (const host in siteConfigMap) {
                ...
                if (isCacheKey(url)) {
                    const ck = extractCacheKey(url);
                    const originalName = getOriginalFilename(ck);
                    const ext = originalName ? getExtFromFilename(originalName) : 'jpg';
                    ...
                    if (!imgData) {
                        ...
                        if (backup && backup.startsWith('data:image/')) {
                            imgData = dataUrlToUint8Array(backup);
                        }
                    }
                    if (imgData) {
            ...
            const configJson = new TextEncoder().encode(JSON.stringify(configData, null, 2));
            imageEntries.push({ name: 'config.json', data: configJson });

            const zipped = await buildZip(imageEntries);
            ...
            if (failedRemoteCount > 0) {
                alert('⚠️ 导出完成，但有 ' + failedRemoteCount + ' 个站点远程图片下载失败。');
            }
        } catch (e) {
    ...
    async function importAllConfigZip() {
        ...
        input.addEventListener('change', async () => {
            ...
            try {
                ...
                const unzipped = await parseZipForImport(new Uint8Array(arrayBuffer));
                ...
                for (const path in unzipped) {
                    if (path.startsWith('images/') && path !== 'images/') {
                        ...
                        const dataUrl = uint8ArrayToDataUrl(unzipped[path], mime);
                        
                        const key = await storeImage(dataUrl, null, fname);
                        if (key) imgCacheKeys[fname] = buildCacheKeyUrl(key);
                    }
                }

                const gRemoteUrl = (configData.global && configData.global.remoteUrl) || '';
                const gLocalFile = configData.global.url;
                const gCacheKeyUrl = (gLocalFile && imgCacheKeys[gLocalFile]) ? imgCacheKeys[gLocalFile] : '';

                if (gRemoteUrl) configData.global.url = gRemoteUrl;
                else if (gCacheKeyUrl) configData.global.url = gCacheKeyUrl;
                else configData.global.url = DEFAULTS.url;

                if (configData.siteConfigMap) {
                    for (const host in configData.siteConfigMap) {
                        const sc = configData.siteConfigMap[host];
                        const sRemoteUrl = sc.remoteUrl || '';
                        const sLocalFile = sc.url || '';
                        const sCacheKeyUrl = (sLocalFile && imgCacheKeys[sLocalFile]) ? imgCacheKeys[sLocalFile] : '';

                        if (sRemoteUrl) sc.url = sRemoteUrl;
                        else if (sCacheKeyUrl) sc.url = sCacheKeyUrl;
                        else delete sc.url;
                        delete sc.remoteUrl;
                    }
                }
    ...
    function applyImportedConfig(data) {
        ...
        configCache = null;
        const gInput = document.getElementById('vie-v83-global-image-url');
        const sInput = document.getElementById('vie-v83-site-image-url');
        if (gInput) {
            const gUrl = getValue('Vie背景图片', DEFAULTS.url);
            gInput.value = (isCacheKey(gUrl) || isDataImageUrl(gUrl)) ? '' : gUrl;
        }
        if (sInput) {
            const importedSiteCfg = getSiteConfig(getHost());
            if (importedSiteCfg && importedSiteCfg.url && !isCacheKey(importedSiteCfg.url) && !isDataImageUrl(importedSiteCfg.url)) {
                sInput.value = importedSiteCfg.url;
            } else {
                sInput.value = '';
            }
        }
    ...
    function registerMenus() {
        ...
        GM_registerMenuCommand(globalCfg.listMode === 'blacklist' ? '切换为白名单模式' : '切换为黑名单模式', () => {
            setGlobal('Vie背景列表模式', getGlobalConfig().listMode === 'blacklist' ? 'whitelist' : 'blacklist');
        });
        ...
        GM_registerMenuCommand(siteCfg && siteCfg.enabled === false ? '当前站点：单独启用' : '当前站点：单独禁用', () => {
            updateCurrentSiteConfig({ enabled: (getSiteConfig(host) || {}).enabled === false });
        });
    }
    ...
    function ensureFloatStyle() {
        ...
        style.textContent = `
#${FLOAT_ID}{position:fixed!important;z-index:2147483647!important;font-family:sans-serif!important;user-select:none!important;display:block!important;visibility:visible!important;}
...
#vie-bg-panel-v83{margin-top:8px!important;width:260px!important;padding:12px!important;border-radius:12px!important;background:rgba(0,0,0,0.85)!important;color:#f0f0f0!important;font-size:12px!important;box-shadow:0 4px 20px rgba(0,0,0,0.5)!important;display:none;max-height:70vh!important;overflow-y:auto!important;}
...
#vie-bg-panel-v83::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.2)!important;border-radius:2px!important;}`;
    ...
    function createFloat() {
        console.log('[浏览器背景] 尝试创建悬浮球...');
        const host = getHost();
        const cfg = mergeConfig(host);
        if (!cfg.floatVisible) {
            console.log('[浏览器背景] 悬浮球已禁用');
        ...
        panelEl.id = 'vie-bg-panel-v83';

        box.innerHTML = '<div id="vie-bg-toggle-v83" title="点击展开/收起；拖动移动">𖣐</div>';

        // 主面板结构：高级设置包含保存、本地图、导入导出、重置
        panelEl.innerHTML = `
        <div class="btns" style="margin-bottom:8px!important;">
            <button id="vie-v83-advanced">⚙️ 高级设置 ▼</button>
        </div>
        <div id="vie-v83-advanced-panel" style="display:none;">
            <hr class="section-divider">
            <div class="section-title">保存配置</div>
            <div class="btns btn-primary">
                <button id="vie-v83-save-global">💾 存全局</button>
                <button id="vie-v83-save-site">💾 存本站</button>
            </div>
            <hr class="section-divider">
            <div class="section-title">本地图片</div>
            <div class="btns">
                <button id="vie-v83-pick-global">🖼️ 全局本地图</button>
                <button id="vie-v83-pick-site">🖼️ 本站本地图</button>
            </div>
            <hr class="section-divider">
            <div class="section-title">导入导出</div>
            <div class="btns btn-export">
                <button id="vie-v83-export">📦 导出压缩包</button>
                <button id="vie-v83-import">📂 导入压缩包</button>
            </div>
            <hr class="section-divider">
            <div class="section-title">重置设置</div>
            <div class="btns btn-danger">
                <button id="vie-v83-reset-global">🔄 全局默认</button>
                <button id="vie-v83-reset-site">🗑️ 清本站</button>
            </div>
        </div>
        <hr class="section-divider">
        ...
        <div class="row">
            ...
            <input id="vie-v83-global-image-url" type="text" placeholder="输入网络图片链接">
        </div>
        <div class="row">
            <div class="lab">当前站点背景图 URL</div>
            <input id="vie-v83-site-image-url" type="text" placeholder="留空则使用全局图片">
        </div>
        <div class="row">
            <div class="lab">透明度 <span id="vie-v83-opacity-txt">${Math.round(cfg.opacity * 100)}%</span></div>
            <input id="vie-v83-opacity" type="range" min="10" max="100" step="1" value="${Math.round(cfg.opacity * 100)}">
        </div>
        <div class="row">
            <div class="lab">背景模糊 <span id="vie-v83-blur-txt">${cfg.blur}px</span></div>
            <input id="vie-v83-blur" type="range" min="0" max="50" step="1" value="${cfg.blur}">
        </div>
        <div class="row">
            <div class="lab">弹层模糊 <span id="vie-v83-native-blur-txt">${cfg.nativeElementBlur}px</span></div>
            <input id="vie-v83-native-blur" type="range" min="0" max="20" step="1" value="${cfg.nativeElementBlur}">
        </div>
        `;
        ...
        section3.innerHTML = `
            ...
            <div class="row">
                <div class="lab">自动弹层模糊 <span id="vie-v83-overlay-blur-txt">${cfg.overlayBlur}px</span></div>
                <input id="vie-v83-overlay-blur" type="range" min="0" max="40" step="1" value="${cfg.overlayBlur}">
            </div>
            <div class="row">
                <div class="lab">自动弹层透明 <span id="vie-v83-overlay-alpha-txt">${cfg.overlayAlpha.toFixed(2)}</span></div>
                <input id="vie-v83-overlay-alpha" type="range" min="0" max="80" step="1" value="${Math.round(cfg.overlayAlpha * 100)}">
            </div>
        `;

        panelEl.appendChild(section3);
        ...
        try {
            ...
            console.error('[浏览器背景] 挂载悬浮球失败:', e);
        }

        cachedFloatNode = box;
        floatShouldExist = true;
        console.log('[浏览器背景] 悬浮球创建成功');

        const toggle = box.querySelector('#vie-bg-toggle-v83');
        const advancedBtn = box.querySelector('#vie-v83-advanced');
        const advPanelEl = box.querySelector('#vie-v83-advanced-panel');

        const globalImageUrlInput = box.querySelector('#vie-v83-global-image-url');
        const siteImageUrlInput = box.querySelector('#vie-v83-site-image-url');
        const opacityEl = box.querySelector('#vie-v83-opacity');
        const blurEl = box.querySelector('#vie-v83-blur');
        const nativeBlurEl = box.querySelector('#vie-v83-native-blur');
        const overlayBlurEl = box.querySelector('#vie-v83-overlay-blur');
        const overlayAlphaEl = box.querySelector('#vie-v83-overlay-alpha');

        const opacityTxt = box.querySelector('#vie-v83-opacity-txt');
        const blurTxt = box.querySelector('#vie-v83-blur-txt');
        const nativeBlurTxt = box.querySelector('#vie-v83-native-blur-txt');
        const overlayBlurTxt = box.querySelector('#vie-v83-overlay-blur-txt');
        const overlayAlphaTxt = box.querySelector('#vie-v83-overlay-alpha-txt');
        ...
        toggle.addEventListener('click', (e) => {
            ...
            if (isHidden) {
                panelCooldown = true;
                if (panelCooldownTimer) clearTimeout(panelCooldownTimer);
                panelCooldownTimer = setTimeout(() => { panelCooldown = false; }, 500);
            }
        });
        ...
        function applyLivePreview() {
            ...
            livePreviewRaf = requestAnimationFrame(() => {
                livePreviewRaf = null;
                const merged = mergeConfig(getHost());
                ...
                injectCSPMeta();
                let finalUrl = previewCfg.url;
                if (isCacheKey(finalUrl)) finalUrl = _currentImageUrl || '';
        ...
        advancedBtn.addEventListener('click', (e) => {
            ...
            advPanelEl.style.display = isHidden ? 'block' : 'none';
        ...
        box.querySelector('#vie-v83-save-site').addEventListener('click', (e) => {
            ...
            setSiteConfig(h, Object.assign({}, getSiteConfig(h) || {}, {
                url: v.siteImageUrl || (getSiteConfig(h) || {}).url,
                opacity: v.opacity, blur: v.blur, nativeElementBlur: v.nativeElementBlur,
                overlayBlur: v.overlayBlur, overlayAlpha: v.overlayAlpha
            }));
            _liveOverlayBlur = null; _liveOverlayAlpha = null;
            applyAgain();
            alert('已保存到当前站点：' + h);
        ...
        box.querySelector('#vie-v83-pick-global').addEventListener('click', (e) => {
            e.stopPropagation();
            pickLocalImage((cacheKeyUrl, file) => {
                setValue('Vie背景图片', cacheKeyUrl);
                configCache = null;
        ...
        box.querySelector('#vie-v83-pick-site').addEventListener('click', (e) => {
            ...
            pickLocalImage((cacheKeyUrl, file) => {
                const h = getHost();
                setSiteConfig(h, Object.assign({}, getSiteConfig(h) || {}, { url: cacheKeyUrl }));
                configCache = null;
                siteImageUrlInput.value = '';
                globalImageUrlInput.value = '';
                applyAgain();
        ...
        box.querySelector('#vie-v83-reset-site').addEventListener('click', (e) => {
            e.stopPropagation();
            if (!confirm('确定清空当前站点的所有单独配置？')) return;
            configCache = null;
            removeSiteConfig(getHost());
            siteImageUrlInput.value = '';
    ...
    function ensureFloatAlive() {
        ...
        if (!el || !(document.body.contains(el) || document.documentElement.contains(el))) {
