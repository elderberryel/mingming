...
#${IMG_LAYER_ID}{position:fixed!important;inset:0!important;z-index:-2147483647!important;pointer-events:none!important;overflow:hidden!important;background:rgba(0,0,0,${darkMask})!important;} 
#${IMG_LAYER_ID} img{width:100%!important;height:100%!important;display:block!important;object-fit:${cfg.mode==='contain'?'contain':cfg.mode==='100% 100%'?'fill':cfg.mode==='auto'?'none':'cover'}!important;object-position:center!important;opacity:1!important;filter:brightness(${cfg.opacity}) blur(${cfg.blur}px)!important;transform:translateZ(0)!important;} 
...
td,th,thead,tbody,tfoot,.rounded-top-2,.rounded-bottom-2,.rounded-2{background:transparent!important;background-color:transparent!important;}`;
    ...
    async function buildZip(entries) {
        ...
        let totalSize = 0;
        processed.forEach(p => {
            totalSize += 30 + p.nameBytes.length + p.compressedSize;
        });
...
`;
    ...
    <div class="btns" style="margin-top:6px!important;">
        ...
        box.querySelector('#vie-v73-save-site').addEventListener('click', (e) => {
            ...
            const siteUrl = v.siteImageUrl.trim();
            const newSiteCfg = Object.assign({}, existingSiteCfg, {
                opacity: v.opacity,
                blur: v.blur,
            ...
            if (siteUrl) {
                newSiteCfg.url = siteUrl;
            } else {
                delete newSiteCfg.url;
            }
            setSiteConfig(h, newSiteCfg);
            _liveOverlayBlur = null; _liveOverlayAlpha = null;
            applyAgain();
            alert('已保存到当前站点：' + h);
        });
