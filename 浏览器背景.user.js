...
td,th,thead,tbody,tfoot,.rounded-top-2,.rounded-bottom-2,.rounded-2{background:transparent!important;background-color:transparent!important;}`;
    ...
    function applyNativeBlur(blurAmount) {
        ...
        if (old) { if (blurAmount <= 0) { old.remove(); cachedNativeBlurStyle = null; return; } }
        if (blurAmount > 0) {
            let s = old;
            if (!s || !document.contains(s)) { s = document.createElement('style'); s.id = NATIVE_BLUR_STYLE_ID; (document.head || document.documentElement).appendChild(s); }
    ...
    function findLikelyOverlays() {
        ...
        const floatEl = document.getElementById(FLOAT_ID), overlays = [];
        const vw = window.innerWidth, vh = window.innerHeight, allElements = document.body.getElementsByTagName('*');
        for (let i = 0; i < allElements.length; i++) {
            ...
            const rect = el.getBoundingClientRect(), z = getZIndex(style), keyword = hasOverlayKeyword(el);
            if (keyword) {
                const overlayBg = hasOverlayBg(style), isFixed = pos === 'fixed', highZ = z >= 40, mediumZ = z >= 10;
                ...
                if ((isFixed && highZ && bigEnough && (overlayBg || keyword)) || (highZ && mediumLarge) || (isFixed && highZ && rect.width >= vw * 0.5 && rect.height >= vh * 0.5)) { overlays.push(el); continue; }
                if (mediumZ && smallLarge) { overlays.push(el); continue; }
            }
            if (z >= 40 && hasOverlayBg(style) && rect.width >= vw * 0.35 && rect.height >= vh * 0.35) overlays.push(el);
        }
    ...
    function applyOverlayEnhance() {
        ...
        const blur = _liveOverlayBlur !== null ? _liveOverlayBlur : cfg.overlayBlur, alpha = _liveOverlayAlpha !== null ? _liveOverlayAlpha : cfg.overlayAlpha;
        ...
        overlays.forEach(el => {
            ...
            const style = getComputedStyle(el), useWhite = isLightBg(style.backgroundColor || '');
            ...
            overlayMarked.add(el); overlayLastApplied.set(el, { blur, alpha });
        });
    }
...
#vie-bg-panel-v83::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.2)!important;border-radius:2px!important;}`;
    ...
    function createFloat() {
        ...
        toggle.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault(); // 阻止移动端默认行为及幽灵点击
            ...
            if (isHidden) {
                // 展开面板时：立即禁用面板的指针事件，防止手指残留或滑动误触滑块
