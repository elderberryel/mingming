...
function getBadgeSize(){const w=window.innerWidth;if(w>=1200)return 100;if(w>=768)return 100;return 52}
function getPanelWidth(){const w=window.innerWidth;if(w>=1200)return 300;if(w>=768)return 300;return 200}
function getDefaultRight(){return Math.round((window.innerWidth-getBadgeSize())/2)}
...
async function detectEngineAuto(){try{const res=await gmFetch({method:'GET',url:'https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh&q=test',timeout:3000});if(res.status===200){console.log('[AutoEngine] Google 可用 → 使用 Google');return'google'}}catch(e){console.log('[AutoEngine] Google 不可用 → 使用 Microsoft')}return'microsoft'}
let targetLang=_targetLang;let autoMode=_autoMode;let excludedHosts=JSON.parse(_excludedHosts);let displayMode=_displayMode;let uiPos=JSON.parse(_pos);
...
async function gmFetchWithRetry(opts,maxRetries=3){let lastError;for(let attempt=0;attempt<=maxRetries;attempt++){try{const result=await gmFetch(opts);if(result.status===429){const waitTime=Math.min(2000*Math.pow(2,attempt),10000);console.warn(`[RateLimit] 429 收到，等待 ${waitTime}ms 后重试 (${attempt+1}/${maxRetries})`);await delay(waitTime);lastError=new Error('429 Too Many Requests');continue}return result}catch(e){lastError=e;if(attempt<maxRetries){const waitTime=Math.min(1000*Math.pow(2,attempt),8000);console.warn(`[Retry] 请求失败，等待 ${waitTime}ms 后重试 (${attempt+1}/${maxRetries})`);await delay(waitTime)}}}throw lastError}
const GoogleHelper_v2={_lastRequestAuthTime:null,_translateAuth:null,_authNotFound:false,_authPromise:null,get translateAuth(){return this._translateAuth},_getAlternativeKey(){return new TextDecoder().decode(new Uint8Array([65,73,122,97,83,121,65,84,66,88,97,106,118,122,81,76,84,68,72,69,81,98,99,112,113,48,73,104,101,48,118,87,68,72,109,79,53,50,48]))},async findAuth(){if(this._authPromise)return await this._authPromise;this._authPromise=new Promise((resolve)=>{let needUpdate=false;if(this._lastRequestAuthTime){const d=new Date();if(this._translateAuth)d.setMinutes(d.getMinutes()-20);else if(this._authNotFound)d.setMinutes(d.getMinutes()-5);else d.setMinutes(d.getMinutes()-1);if(d.getTime()>this._lastRequestAuthTime)needUpdate=true}else{needUpdate=true}if(needUpdate){this._lastRequestAuthTime=Date.now();const altKey=this._getAlternativeKey();GM_xmlhttpRequest({method:'GET',url:'https://translate.googleapis.com/_/translate_http/_/js/k=translate_http.tr.en_US.YusFYy3P_ro.O/am=AAg/d=1/exm=el_conf/ed=1/rs=AN8SPfq1Hb8iJRleQqQc8zhdzXmF9E56eQ/m=el_main',timeout:8000,onload:(r)=>{if(r.responseText&&r.responseText.length>1){const m=r.responseText.match(/['"]x-goog-api-key['"]\s*:\s*['"](\w{39})['"]/i);if(m&&m.length===2){this._translateAuth=m[1];this._authNotFound=false}else{this._authNotFound=true;this._translateAuth=altKey}}else{this._authNotFound=true;this._translateAuth=altKey}resolve()},onerror:()=>{this._translateAuth=altKey;resolve()},ontimeout:()=>{this._translateAuth=altKey;resolve()}})}else{resolve()}});const p=this._authPromise;p.finally(()=>{this._authPromise=null});return await p}};
...
const GoogleHelper={googleTranslateTKK:"448487.932609646",shiftLeftOrRightThenSumOrXor(num,optString){for(let i=0;i<optString.length-2;i+=3){let acc=optString.charAt(i+2);acc=("a"<=acc)?acc.charCodeAt(0)-87:Number(acc);acc=(optString.charAt(i+1)==="+")?num>>>acc:num<<acc;num=(optString.charAt(i)==="+")?(num+acc)&4294967295:num^acc}return num},transformQuery(query){const b=[];let idx=0;for(let i=0;i<query.length;i++){let c=query.charCodeAt(i);if(128>c){b[idx++]=c}else{if(2048>c){b[idx++]=(c>>6)|192}else{if(55296===(c&64512)&&i+1<query.length&&56320===(query.charCodeAt(i+1)&64512)){c=65536+((c&1023)<<10)+(query.charCodeAt(++i)&1023);b[idx++]=(c>>18)|240;b[idx++]=((c>>12)&63)|128}else{b[idx++]=(c>>12)|224}b[idx++]=((c>>6)&63)|128}b[idx++]=(c&63)|128}}return b},calcHash(query){const s=this.googleTranslateTKK.split(".");const tkkIdx=Number(s[0])||0;const tkkKey=Number(s[1])||0;const bytes=this.transformQuery(query);let enc=tkkIdx;for(const item of bytes){enc+=item;enc=this.shiftLeftOrRightThenSumOrXor(enc,"+-a^+6")}enc=this.shiftLeftOrRightThenSumOrXor(enc,"+-3^+b+-f");enc^=tkkKey;if(enc<=0)enc=(enc&2147483647)+2147483648;const n=enc%1000000;return n.toString()+"."+(n^tkkIdx)}};
const Utils={unescapeHTML(t){const d=new DOMParser().parseFromString(t,'text/html');return d.documentElement.textContent}};
...
_transformResponse(result,dontSort){if(result.indexOf("<pre>")!==-1){result=result.replace("<pre>","");const i=result.indexOf(">");result=result.slice(i+1)}const sentences=[];let idx=0;while(true){const s=result.indexOf("<b>",idx);if(s===-1)break;const e=result.indexOf("</b>",s);if(e===-1){sentences.push(result.slice(s+3));break}else{sentences.push(result.slice(s+3,e))}idx=e}result=sentences.length>0?sentences.join(" "):result;result=result.replace(/<\/b>/g,"");let resultArray=[];let lastEnd=0;for(const r of result.matchAll(/(<a i="[0-9]+">)([^<>]*(?=<\/a>))*/g)){const fl=r[0].length,pos=r.index;if(pos>lastEnd){resultArray.push(r[1]+result.slice(lastEnd,pos).replace(/<\/a>/g,"")+(r[2]||""))}else{resultArray.push(r[0])}lastEnd=pos+fl}let indexes;if(resultArray.length>0){indexes=resultArray.map(v=>parseInt(v.match(/[0-9]+(?=>)/g)?.[0])).filter(v=>!isNaN(v));resultArray=resultArray.map(v=>v.slice(v.indexOf(">")+1))}else{resultArray=[result];indexes=[0]}resultArray=resultArray.map(v=>Utils.unescapeHTML(v));if(dontSort)return resultArray;const final=[];for(const j in indexes){if(final[indexes[j]])final[indexes[j]]+=" "+resultArray[j];else final[indexes[j]]=resultArray[j]}return final},
async translate(text,toLang){const to=this._fixLang(toLang);await GoogleHelper_v2.findAuth();if(!GoogleHelper_v2.translateAuth)throw new Error('No auth');const r=await gmFetchWithRetry({method:'POST',url:'https://translate-pa.googleapis.com/v1/translateHtml',headers:{'Content-Type':'application/json+protobuf','X-Goog-Api-Key':GoogleHelper_v2.translateAuth},data:JSON.stringify([[[text],"auto",to],"te"])},2);if(r.status!==200)throw new Error('v2 error: '+r.status);const data=JSON.parse(r.responseText);if(data&&data[0]){const raw=Array.isArray(data[0])?data[0][0]:data[0];const parsed=this._transformResponse(raw,false);return parsed[0]||raw}throw new Error('v2 empty')},
async translateBatch(texts,toLang){const to=this._fixLang(toLang);await GoogleHelper_v2.findAuth();if(!GoogleHelper_v2.translateAuth)throw new Error('No auth');const r=await gmFetchWithRetry({method:'POST',url:'https://translate-pa.googleapis.com/v1/translateHtml',headers:{'Content-Type':'application/json+protobuf','X-Goog-Api-Key':GoogleHelper_v2.translateAuth},data:JSON.stringify([[texts,"auto",to],"te"])},2);if(r.status!==200)throw new Error('v2 batch error: '+r.status);const data=JSON.parse(r.responseText);if(data&&data[0]&&Array.isArray(data[0])){return data[0].map(item=>{const p=this._transformResponse(item,false);return p[0]||item})}if(data&&data[0]){const p=this._transformResponse(Array.isArray(data[0])?data[0][0]:data[0],false);return[p[0]]}throw new Error('v2 batch empty')}
},
google_legacy:{
name:'Google (Legacy)',
async translate(text,toLang){const tk=GoogleHelper.calcHash(text);const r=await gmFetchWithRetry({method:'GET',url:'https://translate.googleapis.com/translate_a/single?client=webapp&sl=auto&tl='+toLang+'&hl='+toLang+'&dt=t&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=at&ie=UTF-8&oe=UTF-8&otf=1&ssel=0&tsel=0&kc=7&tk='+tk+'&q='+encodeURIComponent(text)},2);if(r.status!==200)return await this._gtx(text,toLang);const data=JSON.parse(r.responseText);return data[0].filter(s=>s&&s[0]).map(s=>s[0]).join('')},
async _gtx(text,to){const r=await gmFetchWithRetry({method:'GET',url:'https://translate.googleapis.com/translate_a/single?client=gtx&dt=t&sl=auto&tl='+to+'&q='+encodeURIComponent(text)},2);if(r.status!==200)throw new Error('gtx error');const data=JSON.parse(r.responseText);return data[0].filter(s=>s&&s[0]).map(s=>s[0]).join('')}
},
google:{
name:'Google (Auto)',
...
async translate(text,toLang){const to=this.langCode(toLang);const r=await gmFetchWithRetry({method:'POST',url:`https://edge.microsoft.com/translate/translatetext?from=&to=${to}&isEnterpriseClient=false`,headers:{'Content-Type':'application/json'},data:JSON.stringify([text])},2);if(r.status!==200)throw new Error('MS Edge error: '+r.status);return JSON.parse(r.responseText)[0].translations[0].text},
async translateBatch(texts,toLang){const to=this.langCode(toLang);const results=[];for(let b=0;b<texts.length;b+=MS_BATCH_SIZE){const chunk=texts.slice(b,b+MS_BATCH_SIZE);try{const r=await gmFetchWithRetry({method:'POST',url:`https://edge.microsoft.com/translate/translatetext?from=&to=${to}&isEnterpriseClient=false`,headers:{'Content-Type':'application/json'},data:JSON.stringify(chunk)},2);if(r.status===200){JSON.parse(r.responseText).forEach(item=>results.push(item.translations[0].text))}else{for(let i=0;i<chunk.length;i++)results.push(null)}}catch(e){for(let i=0;i<chunk.length;i++)results.push(null)}}return results}
},
tencent:{
name:'Tencent',_clientKey:null,
getClientKey(){if(this._clientKey)return this._clientKey;let uid=window.crypto?.randomUUID?.()||(Math.random().toString(36).substring(2,15)+Math.random().toString(36).substring(2,15));this._clientKey='browser-chrome-120.0-Windows_10-'+uid+'-'+Date.now();return this._clientKey},
langCode(l){const m={'zh':'zh','zh-CN':'zh','zh-TW':'zh-TW'};return m[l]||l},
async translate(text,toLang){const to=this.langCode(toLang);const r=await gmFetchWithRetry({method:'POST',url:'https://transmart.qq.com/api/imt',headers:{'Content-Type':'application/json'},data:JSON.stringify({header:{fn:'auto_translation',session:'',client_key:this.getClientKey(),user:''},type:'plain',model_category:'normal',text_domain:'general',source:{lang:'auto',text_list:[text]},target:{lang:to}})},2);if(r.status!==200)throw new Error('Tencent error');return JSON.parse(r.responseText).auto_translation[0]},
async translateBatch(texts,toLang){const to=this.langCode(toLang);const r=await gmFetchWithRetry({method:'POST',url:'https://transmart.qq.com/api/imt',headers:{'Content-Type':'application/json'},data:JSON.stringify({header:{fn:'auto_translation',session:'',client_key:this.getClientKey(),user:''},type:'plain',model_category:'normal',text_domain:'general',source:{lang:'auto',text_list:texts},target:{lang:to}})},2);if(r.status!==200)throw new Error('Tencent batch error');return JSON.parse(r.responseText).auto_translation}
},
deepseek:{
...
const SCRIPT_PATTERNS={latin:/^[\p{Script=Latin}\s\d\p{P}\p{S}]+$/u,cyrillic:/^[\p{Script=Cyrillic}\s\d\p{P}\p{S}]+$/u,arabic:/^[\p{Script=Arabic}\s\d\p{P}\p{S}]+$/u,greek:/^[\p{Script=Greek}\s\d\p{P}\p{S}]+$/u,hebrew:/^[\p{Script=Hebrew}\s\d\p{P}\p{S}]+$/u,devanagari:/^[\p{Script=Devanagari}\s\d\p{P}\p{S}]+$/u,thai:/^[\p{Script=Thai}\s\d\p{P}\p{S}]+$/u,lao:/^[\p{Script=Lao}\s\d\p{P}\p{S}]+$/u,khmer:/^[\p{Script=Khmer}\s\d\p{P}\p{S}]+$/u,myanmar:/^[\p{Script=Myanmar}\s\d\p{P}\p{S}]+$/u,bengali:/^[\p{Script=Bengali}\s\d\p{P}\p{S}]+$/u,tamil:/^[\p{Script=Tamil}\s\d\p{P}\p{S}]+$/u,telugu:/^[\p{Script=Telugu}\s\d\p{P}\p{S}]+$/u,kannada:/^[\p{Script=Kannada}\s\d\p{P}\p{S}]+$/u,malayalam:/^[\p{Script=Malayalam}\s\d\p{P}\p{S}]+$/u,gujarati:/^[\p{Script=Gujarati}\s\d\p{P}\p{S}]+$/u,gurmukhi:/^[\p{Script=Gurmukhi}\s\d\p{P}\p{S}]+$/u,sinhala:/^[\p{Script=Sinhala}\s\d\p{P}\p{S}]+$/u,georgian:/^[\p{Script=Georgian}\s\d\p{P}\p{S}]+$/u,armenian:/^[\p{Script=Armenian}\s\d\p{P}\p{S}]+$/u};
...
const LANG_SCRIPT={ru:'cyrillic',uk:'cyrillic',bg:'cyrillic',sr:'cyrillic',be:'cyrillic',mk:'cyrillic',kk:'cyrillic',ky:'cyrillic',mn:'cyrillic',tg:'cyrillic',tt:'cyrillic',el:'greek',he:'hebrew',ar:'arabic',fa:'arabic',ur:'arabic',ps:'arabic',ckb:'arabic',sd:'arabic',ug:'arabic',hi:'devanagari',mr:'devanagari',ne:'devanagari',sa:'devanagari',mai:'devanagari',bho:'devanagari',th:'thai',lo:'lao',km:'khmer',my:'myanmar',bn:'bengali',as:'bengali',ta:'tamil',te:'telugu',kn:'kannada',ml:'malayalam',gu:'gujarati',pa:'gurmukhi',si:'sinhala',ka:'georgian',hy:'armenian'};
const LATIN_LANGS=new Set(['en','fr','de','es','it','pt','nl','pl','cs','sk','hu','ro','hr','sl','lt','lv','et','fi','sv','da','no','is','tr','id','ms','vi','ca','gl','eu','cy','ga','gd','lb','af','sw','ha','ig','yo','zu','xh','sn','st','so','mg','ny','rw','eo','la','co','fy','sq','az','uz','tk','ceb','fil','jv','su','hmn','ht','oc','sc','scn','br','war','wo']);
const _langRegex={};
function getLangRegex(lang){if(lang in _langRegex)return _langRegex[lang];let re=CJK_PATTERNS[lang]||null;if(!re){const script=LANG_SCRIPT[lang]||(LATIN_LANGS.has(lang)?'latin':null);if(script)re=SCRIPT_PATTERNS[script]}_langRegex[lang]=re||null;return _langRegex[lang]}
...
function localizeDateValue(value){if(!DATE_TOSTRING_RE.test(value)&&!DATE_ISO_RE.test(value))return null;const d=new Date(value);if(isNaN(d.getTime()))return null;try{const s=new Intl.DateTimeFormat(targetLang,{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZoneName:'short'}).format(d);return s===value?null:s}catch(_){return null}}
...
function lookupExifKey(key){if(!exifDictEnabled())return null;if(Object.prototype.hasOwnProperty.call(EXIF_KEY_MAP,key))return EXIF_KEY_MAP[key];const lower=key.toLowerCase();for(const k in EXIF_KEY_MAP){if(k.toLowerCase()===lower)return EXIF_KEY_MAP[k]}return null}
function lookupExifValue(value){if(!exifDictEnabled())return null;const norm=normalizeExifValue(value);return Object.prototype.hasOwnProperty.call(EXIF_VALUE_MAP,norm)?EXIF_VALUE_MAP[norm]:null}
...
let msMonitorTimer=null;
function addEvent(target,type,listener,options={}){if(!abortController)abortController=new AbortController();target.addEventListener(type,listener,{...options,signal:abortController.signal})}
function cleanup(){if(abortController){abortController.abort();abortController=null}if(cacheSaveTimer){clearInterval(cacheSaveTimer);cacheSaveTimer=null}if(msMonitorTimer){clearInterval(msMonitorTimer);msMonitorTimer=null}if(processTimer){clearTimeout(processTimer);processTimer=null}if(mutationRafId){clearTimeout(mutationRafId);mutationRafId=null}if(visibilityObserver){visibilityObserver.disconnect();visibilityObserver=null}if(mutationObserver){mutationObserver.disconnect();mutationObserver=null}const ui=document.querySelector('.translate-ui');if(ui)ui.remove();const style=document.getElementById('tu-custom-styles');if(style)style.remove();pendingQueue.length=0;pendingMutationRoots.clear();translatedNodes.clear();translatedElements.clear();statusEl=null}
...
.tu-btn{
  position:relative;
  border-radius:50%;
  border:1px double rgba(255,255,255,0.22);
  background:rgba(28,30,44,0.38);
  color:#fff;
  cursor:grab;
...
.tu-btn::before{
  ...
  border:1px double rgba(0,0,0,0.75);
  filter:blur(5px);z-index:-1;
}
.tu-btn svg{position:relative;z-index:1;filter:drop-shadow(0 1px 2px rgba(0,0,0,0.5))}
.tu-btn:active{cursor:grabbing;transform:scale(0.9)}
.tu-btn.active{background:rgba(31,90,58,0.5);color:#d4f0d4}
.tu-panel{
  ...
  padding:12px;display:none;color:#eef2ff;font-size:13px;margin-bottom:8px;
...
.tu-panel select,.tu-panel input{
  width:100%;padding:8px 12px;border-radius:12px;font-size:12px;
  color:#f0f3fa;outline:none;appearance:auto;
  background:rgba(255,255,255,0.07);
  border:1px double rgba(255,255,255,0.20);
  backdrop-filter:blur(6px) saturate(160%);
  -webkit-backdrop-filter:blur(6px) saturate(160%);
  box-shadow:
    inset 1.5px -1.5px 1px -1px rgba(255,255,255,0.55),
    inset -1.5px 1.5px 1px -1px rgba(255,255,255,0.45),
    inset 0 0 3px rgba(0,0,0,0.30),
    0 4px 12px rgba(0,0,0,0.15);
  transition:all .2s;
}
.tu-panel select:focus,.tu-panel input:focus{
  border-color:rgba(120,160,255,0.5);
  box-shadow:
    inset 1.5px -1.5px 1px -1px rgba(255,255,255,0.70),
    inset -1.5px 1.5px 1px -1px rgba(255,255,255,0.60),
    inset 0 0 3px rgba(0,0,0,0.30),
    0 0 0 2px rgba(90,124,191,0.25),
    0 4px 12px rgba(0,0,0,0.20);
}
.tu-panel option{background:#2a2e3a;color:#f0f3fa}
.tu-ai-config{
  background:rgba(255,255,255,0.04);padding:8px 10px;border-radius:14px;
  margin-top:6px;border:1px double rgba(255,255,255,0.10);display:none;
  backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
}
.tu-ai-config.show{display:block}
.tu-ai-config input{margin-top:6px;background:rgba(255,255,255,0.07)}
.tu-status{
  margin-top:10px;padding:8px;
  background:rgba(255,255,255,0.04);
  border:1px double rgba(255,255,255,0.10);
  border-radius:12px;font-size:11px;color:#b0becc;text-align:center;
...
.tu-modes{
  ...
  background:rgba(255,255,255,0.04);
  border:1px double rgba(255,255,255,0.10);
  border-radius:14px;padding:3px;gap:4px;
  backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
  box-shadow:inset 0 1px 2px rgba(0,0,0,0.2);
}
.tu-modes button{
  flex:1;padding:7px 0;border:none;border-radius:10px;font-size:11px;
  cursor:pointer;background:transparent;color:#a0aec0;transition:all .2s;font-weight:500;
}
.tu-modes button.on{
  background:rgba(90,124,191,0.35);color:#fff;
  backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
  box-shadow:
    inset 1px -1px 1px -1px rgba(255,255,255,0.60),
    inset -1px 1px 1px -1px rgba(255,255,255,0.50),
    inset 0 0 3px rgba(0,0,0,0.30),
    0 2px 8px rgba(0,0,0,0.20);
}
.tu-modes button:hover:not(.on){background:rgba(255,255,255,0.06);color:#e2e8ff}
.tu-row{display:flex;gap:8px;margin-top:12px}
.tu-row button{
  flex:1;padding:9px 0;border-radius:12px;font-size:12px;cursor:pointer;font-weight:500;
  position:relative;overflow:hidden;isolation:isolate;
  border:1px double rgba(255,255,255,0.15);
  background:rgba(255,255,255,0.05);color:#dce6f5;
...
.tu-row button:hover{transform:translateY(-1px)}
.tu-row button:active{transform:translateY(0) scale(0.98)}

.tu-row .tu-restore{background:rgba(255,255,255,0.05);color:#cbd5e6;border-color:rgba(255,255,255,0.15)}
.tu-row .tu-go{background:rgba(44,90,124,0.35);color:#fff;border-color:rgba(120,180,240,0.30)}
.tu-row .tu-go:hover{background:rgba(54,113,160,0.50)}
.tu-row .tu-exclude{background:rgba(107,46,58,0.30);color:#ffcdd6;border-color:rgba(220,120,140,0.25)}
.tu-row .tu-exclude:hover{background:rgba(138,58,72,0.45)}
.tu-row .tu-io{background:rgba(255,255,255,0.05);color:#dce6f5}
