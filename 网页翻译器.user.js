...
(async () => {
  ...
  try {
    ...
    console.log(`[Cache] 从持久化存储加载 ${Object.keys(cacheData).length} 条缓存`);
  } catch (e) {
    console.warn('[Cache] 缓存解析失败，使用空缓存');
    cacheData = {};
  }
  ...
  function saveCache() {
    ...
    try {
      ...
      cacheModified = false;
      console.log(`[Cache] 持久化保存 ${cache.size} 条缓存`);
    } catch (e) {
      ...
      if (e.message && e.message.includes('quota')) {
        ...
        try {
          ...
          console.warn(`[Cache] 存储空间不足，已清理到 ${cache.size} 条`);
        } catch (_) {}
      }
    }
  }
  ...
  async function aiBatchTranslate(texts, toLang, apiKey, model, apiUrl, engineName) {
    ...
    if (texts.length === 1) {
      const result = await aiSingleTranslate(texts[0], toLang, apiKey, model, apiUrl, engineName);
      return [result];
    }
    ...
    for (let i = 0; i < texts.length; i += AI_BATCH_SIZE) {
      ...
      if (batch.length === 1) {
        try {
          const result = await aiSingleTranslate(batch[0], toLang, apiKey, model, apiUrl, engineName);
          allResults.push(result);
        } catch (e) {
          console.warn(`[AI Batch] 单条翻译失败:`, e);
...
${numberedTexts}`;
      ...
      try {
        ...
        if (r.status !== 200) {
          throw new Error(`${engineName} Error: ${r.status}`);
        }

        const data = JSON.parse(r.responseText);
        if (data.error) {
          throw new Error(`${engineName} API: ${data.error.message}`);
        }

        const responseText = data.choices[0].message.content.trim();
        const batchResults = parseNumberedResponse(responseText, batch.length);
        allResults.push(...batchResults);

        console.log(`[AI Batch] ${engineName} 批量翻译成功: ${batch.length} 条`);
      } catch (e) {
        console.warn(`[AI Batch] ${engineName} 批量翻译失败，回退到单条翻译:`, e);
        for (const text of batch) {
          try {
            const result = await aiSingleTranslate(text, toLang, apiKey, model, apiUrl, engineName);
            allResults.push(result);
          } catch (e2) {
            console.warn(`[AI Batch] 单条翻译也失败:`, e2);
            allResults.push(null);
          }
        }
      }

      if (i + AI_BATCH_SIZE < texts.length) {
        await delay(500);
      }
    }
  ...
  function parseNumberedResponse(responseText, expectedCount) {
    ...
    if (results.every(r => r === null)) {
      console.log('[AI Batch] 编号解析失败，尝试用空行分隔');
      const parts = responseText.split(/\n\s*\n/).filter(s => s.trim());
      for (let i = 0; i < Math.min(parts.length, expectedCount); i++) {
        results[i] = parts[i].trim().replace(/^\[\d+\]\s*/, '');
      }
    }

    if (results.every(r => r === null)) {
      console.log('[AI Batch] 空行分隔也失败，按行分配');
      const linesArr = responseText.split('\n').filter(s => s.trim());
      for (let i = 0; i < Math.min(linesArr.length, expectedCount); i++) {
        results[i] = linesArr[i].trim().replace(/^\[\d+\]\s*/, '');
      }
    }
  ...
  const Engine = {
    ...
    // ★ 替换为 KISS Translator 的微软 Edge 免 Token 接口
    microsoft: {
      name: 'Microsoft (Edge)',
      langCode(l) { 
      ...
      async translateBatch(texts, toLang) {
        ...
        for (let b = 0; b < texts.length; b += 25) {
          ...
          try {
            ...
            console.warn('[MS Edge Batch] Error:', e);
            for (let i = 0; i < chunk.length; i++) results.push(null);
          }
        }
        return results;
    ...
    deepseek: {
      ...
      isAI: true,
      async translate(text, toLang) {
        return await aiSingleTranslate(
          text, toLang,
          aiConfig.deepseek.key,
  ...
  async function translate(text) {
    ...
    try {
      ...
      if (currentEngine === 'microsoft') {
        console.warn('[Translate] Microsoft 翻译失败，尝试切换 Tencent...', e.message);
        try {
          const result = await Engine.tencent.translate(trimmed, targetLang);
          if (result && result !== trimmed) {
            console.log('[Translate] Microsoft → Tencent 切换成功');
          ...
          console.warn('[Translate] Tencent 也失败:', e2.message);
        }
      }
      // 通用回退
  ...
  async function batchTranslate(texts) {
    ...
    if (engine.isAI && engine.translateBatch) {
      console.log(`[BatchTranslate] AI引擎批量翻译: ${uniqueTexts.length} 条`);
      try {
        const batchResults = await engine.translateBatch(uniqueTexts, targetLang);
        if (batchResults) {
          for (let j = 0; j < batchResults.length; j++) {
            if (batchResults[j] && batchResults[j] !== uniqueTexts[j]) {
              fillResults(uniqueTexts[j], batchResults[j]);
            }
          }
        }
        return results;
      } catch (e) {
        console.error("[BatchTranslate] AI批量翻译失败:", e);
      }
    }
      ...
      try {
        ...
        for (let b = 0; b < uniqueTexts.length; b += BATCH_SIZE) {
          ...
          try {
            ...
            // ★ 微软批量失败时切换腾讯并重试
            if (currentEngine === 'microsoft') {
              console.warn('[BatchTranslate] Microsoft 批量失败，尝试 Tencent...');
              try {
                const tencentResults = await Engine.tencent.translateBatch(chunkTexts, targetLang);
                if (tencentResults) {
                  for (let j = 0; j < tencentResults.length; j++) {
                    if (tencentResults[j] && tencentResults[j] !== chunkTexts[j]) {
                      fillResults(chunkTexts[j], tencentResults[j]);
                    }
                  }
                  // 批量切换成功
                  console.log('[BatchTranslate] Microsoft → Tencent 批量切换成功');
                  currentEngine = 'tencent';
                  GM_setValue('engine', 'tencent');
                  const engineSel = document.getElementById('tuEngine');
                  if (engineSel) engineSel.value = 'tencent';
                ...
                console.warn('[BatchTranslate] Tencent 批量也失败:', e2);
              }
            }
            // 回退到逐条翻译
    ...
    for (let i = 0; i < uniqueTexts.length; i += concurrency) {
      ...
      await Promise.allSettled(batch.map(async (text, idx) => {
        try {
          ...
          if (currentEngine === 'microsoft') {
            try {
              const r2 = await Engine.tencent.translate(text, targetLang);
              if (r2) {
                fillResults(text, r2);
                currentEngine = 'tencent';
                GM_setValue('engine', 'tencent');
      ...
      if (requestDelay > 0 && i + concurrency < uniqueTexts.length) {
  ...
  function collectTextNodes(root) {
    ...
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        ...
        if (node._tuTranslated) return NodeFilter.FILTER_REJECT; // 修改为检查节点本身
    ...
    for (const el of elements) {
      if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
        ...
        const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
          acceptNode(node) {
            if (shouldSkip(node.parentElement)) return NodeFilter.FILTER_REJECT;
            if (node._tuTranslated) return NodeFilter.FILTER_REJECT; // 改为检查文本节点
            return NodeFilter.FILTER_ACCEPT;
          }
        });
        while (walker.nextNode()) nodes.push(walker.currentNode);
  ...
  function scanAndObserve(root) {
    ...
    nodes.forEach(node => {
      ...
      if (p) parents.add(p); // 移除对 p.dataset.translated 的判断
    });
  ...
  function restorePage() {
    ...
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        return (node._tuOriginalText !== undefined) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
      }
    });
    ...
    nodesToRestore.forEach(node => {
      node.textContent = node._tuOriginalText;
  ...
  // ══════════════════════════════════════════════════════════
  // 微软引擎状态定时监测 (修改为直接发送测试请求，因为 Edge 接口无需 Token)
  // ══════════════════════════════════════════════════════════
  ...
  function startMicrosoftMonitor() {
    ...
    msMonitorTimer = setInterval(async () => {
      ...
      try {
        // 使用简单的翻译请求代替原来的 token 获取
        await Engine.microsoft.translate('test', 'zh-CN');
      } catch (e) {
        console.warn('[Monitor] 微软引擎心跳失败，尝试切换腾讯...');
        try {
          ...
          console.log('[Monitor] 已切换到腾讯引擎');
        } catch (e2) {
          console.error('[Monitor] 腾讯也不可用:', e2);
        }
      }
    }, 5 * 60 * 1000);
  }
  ...
  async function init() {
    ...
    // ★ 启动时清理可能由于上次刷新残留的 data-translated 和 originalText 标记
    document.querySelectorAll('[data-translated]').forEach(el => {
      if (el.dataset.originalText) {
        for (const child of el.childNodes) {
          if (child.nodeType === Node.TEXT_NODE && child._tuOriginalText === undefined) {
    ...
    ui.innerHTML =
      '<div class="tu-panel" id="tuPanel">' +
        '<label>翻译引擎</label>' +
        '<select id="tuEngine">' +
          '<option value="microsoft">Microsoft Edge (默认)</option>' +
        ...
        '<div id="tuAiConfig" class="tu-ai-config">' +
          ...
          '<label>模型名称</label>' +
          '<input type="text" id="tuModel" placeholder="例如: deepseek-chat">' +
        '</div>' +
        '<label>目标语言</label>' +
        ...
        '<div class="tu-row">' +
          ...
          '<button class="tu-io" id="tuClearCache">清缓存</button>' +
        '</div>' +
        '<div class="tu-row">' +
    ...
    // 保留 updateStatus 的引用，供全局使用
    window._tuUpdateStatus = function updateStatus(msg) {
    ...
    modesEl.addEventListener('click', async (e) => {
      ...
      if (m === 'original') {
        ...
        window._tuUpdateStatus('显示原文');
      } else {
        restorePage();
        btn.classList.add('active');
        window._tuUpdateStatus(m === 'bilingual' ? '双语翻译中...' : '翻译中...');
        scanAndObserve(document.body);
      }
    });
    ...
    document.getElementById('tuRestore').addEventListener('click', () => {
      ...
      restorePage(); window._tuUpdateStatus('已还原');
    });
    ...
    document.getElementById('tuClearCache').addEventListener('click', () => {
      if (confirm('确定要清除所有翻译缓存吗？')) {
        clearCache();
        window._tuUpdateStatus('缓存已清空');
      }
    });
    ...
    document.getElementById('tuExport').addEventListener('click', () => {
      ...
      const data = {
        ...
        glm: aiConfig.glm,
        translationCache: Object.fromEntries(cache)
      };
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (imported.translationCache) {
            GM_setValue('translationCache', JSON.stringify(imported.translationCache));
          }
    ...
    if (autoMode && !isPageInTargetLang() && displayMode !== 'original') {
      queueMicrotask(async () => {
        window._tuUpdateStatus(displayMode === 'bilingual' ? '双语翻译中...' : '自动翻译中...');
        scanAndObserve(document.body);
      });
    }
  }

  // ★ 供全局调用的 updateStatus（因为 translate 函数在 init 之前定义）
  function updateStatus(msg) {
    if (window._tuUpdateStatus) window._tuUpdateStatus(msg);
