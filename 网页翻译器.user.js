...
(async () => {
  ...
  const [
    ...
    GM_getValue('uiPos', JSON.stringify({})),
  ...
  const AI_CONCURRENCY_LIMIT = 1;
  const AI_REQUEST_DELAY = 300;
  ...
  async function gmFetchWithRetry(opts, maxRetries = 3) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const result = await gmFetch(opts);
        if (result.status === 429) {
          const waitTime = Math.min(2000 * Math.pow(2, attempt), 10000);
          console.warn(`[RateLimit] 429 收到，等待 ${waitTime}ms 后重试 (${attempt + 1}/${maxRetries})`);
        ...
        if (attempt < maxRetries) {
          ...
          console.warn(`[Retry] 请求失败，等待 ${waitTime}ms 后重试 (${attempt + 1}/${maxRetries})`);
          await delay(waitTime);
        }
      }
    }
  ...
  const CACHE_SAVE_INTERVAL = 30000;
  let cacheModified = false;

  let cacheData = {};
  try {
    cacheData = JSON.parse(_savedCache);
  } catch (e) {
    cacheData = {};
  }
  ...
  function cacheSet(key, value) {
    if (cache.size >= MAX_CACHE) {
      const deleteCount = Math.floor(MAX_CACHE * 0.2);
      const keys = cache.keys();
      for (let i = 0; i < deleteCount; i++) { cache.delete(keys.next().value); }
    }
    cache.set(key, value);
    cacheModified = true;
  }
  ...
  function saveCache() {
    ...
    try {
      ...
      GM_setValue('translationCache', JSON.stringify(cacheObj));
      cacheModified = false;
    } catch (e) {
      if (e.message && e.message.includes('quota')) {
        ...
        const keys = cache.keys();
        for (let i = 0; i < deleteCount; i++) { cache.delete(keys.next().value); }
        try { GM_setValue('translationCache', JSON.stringify(Object.fromEntries(cache))); } catch (_) {}
      }
    }
  }
  ...
  async function aiSingleTranslate(text, toLang, apiKey, model, apiUrl, engineName) {
    ...
    const r = await gmFetchWithRetry({
      method: 'POST', url: apiUrl,
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + apiKey },
      data: JSON.stringify({ model: model, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: 0.1 })
    });
  ...
  async function aiBatchTranslate(texts, toLang, apiKey, model, apiUrl, engineName) {
    ...
    for (let i = 0; i < texts.length; i += AI_BATCH_SIZE) {
      ...
      if (batch.length === 1) {
        try { allResults.push(await aiSingleTranslate(batch[0], toLang, apiKey, model, apiUrl, engineName)); } 
        catch (e) { allResults.push(null); }
        continue;
      }
      ...
      try {
        ...
        const data = JSON.parse(r.responseText);
        if (data.error) throw new Error(`${engineName} API: ${data.error.message}`);
        allResults.push(...parseNumberedResponse(data.choices[0].message.content.trim(), batch.length));
      } catch (e) {
        for (const text of batch) {
          try { allResults.push(await aiSingleTranslate(text, toLang, apiKey, model, apiUrl, engineName)); } 
          catch (e2) { allResults.push(null); }
        }
      }
  ...
  const Engine = {
    ...
    microsoft: {
      ...
      async translate(text, toLang) {
        ...
        const r = await gmFetch({ method: 'POST', url: `https://edge.microsoft.com/translate/translatetext?from=&to=${to}&isEnterpriseClient=false`, headers: { 'Content-Type': 'application/json' }, data: JSON.stringify([text]) });
        if (r.status !== 200) throw new Error('MS Edge error: ' + r.status);
        return JSON.parse(r.responseText)[0].translations[0].text;
      },
      async translateBatch(texts, toLang) {
        const to = this.langCode(toLang); const results = [];
        for (let b = 0; b < texts.length; b += 25) {
          const chunk = texts.slice(b, b + 25);
          try {
            const r = await gmFetch({ method: 'POST', url: `https://edge.microsoft.com/translate/translatetext?from=&to=${to}&isEnterpriseClient=false`, headers: { 'Content-Type': 'application/json' }, data: JSON.stringify(chunk) });
      ...
      if (currentEngine === 'microsoft') {
        try {
          ...
          if (result && result !== trimmed) {
            ...
            const engineSel = document.getElementById('tuEngine'); if (engineSel) engineSel.value = 'tencent';
  ...
  function initObserver() {
    ...
    visibilityObserver = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) { visibilityObserver.unobserve(entry.target); pendingQueue.push(entry.target); }
      }
  ...
  function restorePage() {
    ...
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, { acceptNode(node) { return (node._tuOriginalText !== undefined) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT; } });
    const nodesToRestore = []; while(walker.nextNode()) nodesToRestore.push(walker.currentNode);
    nodesToRestore.forEach(node => { node.textContent = node._tuOriginalText; node._tuTranslated = false; delete node._tuOriginalText; });
    document.querySelectorAll('[data-translated]').forEach(el => { if (el.dataset.originalPlaceholder) { el.placeholder = el.dataset.originalPlaceholder; delete el.dataset.originalPlaceholder; } delete el.dataset.translated; });
  }
  ...
  function updateUIPos(ui) {
    ...
    const btn = document.getElementById('tuBtn'); if (btn) { btn.style.width = badgeSize + 'px'; btn.style.height = badgeSize + 'px'; }
    clampUiPos(ui);
  }
  ...
  async function init() {
    ...
    if (!document.getElementById('tu-custom-styles')) {
      ...
      styleEl.textContent = `
        ...
        .tu-panel select, .tu-panel input{width:100%;padding:6px 10px;border:1px solid #3a3f4e;border-radius:10px;font-size:12px;background:#2a2e3a;color:#f0f3fa;outline:none;appearance:auto;transition:all .15s}
        ...
        .tu-modes button{flex:1;padding:6px 0;border:none;border-radius:8px;font-size:11px;cursor:pointer;background:transparent;color:#a0aec0;transition:all .2s;font-weight:500}
        ...
        .tu-row button{flex:1;padding:8px 0;border:none;border-radius:10px;font-size:12px;cursor:pointer;transition:all .2s;font-weight:500}
    ...
    window._tuUpdateStatus = function updateStatus(msg) { if (statusEl) statusEl.textContent = msg + ' · 缓存: ' + cache.size; };

    function updateAiConfigUI() {
      const selectedEngine = engineSelect.value; const isAI = Engine[selectedEngine]?.isAI;
      if (isAI) {
        aiConfigDiv.classList.add('show');
        if (selectedEngine === 'deepseek') { apiKeyInput.value = aiConfig.deepseek.key; modelInput.value = aiConfig.deepseek.model; modelInput.placeholder = "deepseek-chat"; } 
        else if (selectedEngine === 'glm') { apiKeyInput.value = aiConfig.glm.key; modelInput.value = aiConfig.glm.model; modelInput.placeholder = "glm-4-flash"; }
    ...
    addEvent(engineSelect, 'change', async () => {
      currentEngine = engineSelect.value; GM_setValue('engine', currentEngine); updateAiConfigUI(); cache.clear();
      window._tuUpdateStatus('切换至: ' + (Engine[currentEngine] ? Engine[currentEngine].name : currentEngine));
      if (displayMode !== 'original' && autoMode) { restorePage(); window._tuUpdateStatus('正在重新翻译...'); scanAndObserve(document.body); }
    });
    ...
    addEvent(langSelect, 'change', async () => {
      targetLang = langSelect.value; GM_setValue('targetLang', targetLang); cache.clear();
      window._tuUpdateStatus('语种切为: ' + (ALL_LANGUAGES[targetLang] || targetLang));
      if (displayMode !== 'original' && autoMode) { restorePage(); window._tuUpdateStatus('正在更新翻译...'); scanAndObserve(document.body); }
    });

    addEvent(modesEl, 'click', async (e) => {
      var b = e.target.closest('button[data-m]'); if (!b) return; var m = b.dataset.m; if (m === displayMode) return;
      ...
      if (m === 'original') { restorePage(); btn.classList.remove('active'); window._tuUpdateStatus('显示原文'); } 
      else { restorePage(); btn.classList.add('active'); window._tuUpdateStatus(m === 'bilingual' ? '双语翻译中...' : '翻译中...'); scanAndObserve(document.body); }
    });

    addEvent(document.getElementById('tuGo'), 'click', async () => {
      ...
      autoMode = true; GM_setValue('autoMode', true); restorePage(); cache.clear();
      window._tuUpdateStatus('翻译中...'); scanAndObserve(document.body);
      setTimeout(() => window._tuUpdateStatus('已开始处理可见区域...'), 100);
    });

    addEvent(document.getElementById('tuRestore'), 'click', () => { panel.classList.remove('show'); btn.classList.remove('active'); autoMode = false; GM_setValue('autoMode', false); restorePage(); window._tuUpdateStatus('已还原'); });
    ...
    addEvent(document.getElementById('tuClearCache'), 'click', () => { if (confirm('确定要清除所有翻译缓存吗？')) { clearCache(); window._tuUpdateStatus('缓存已清空'); } });
    ...
    addEvent(document.getElementById('tuExport'), 'click', () => {
      ...
      const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = "翻译配置.json"; a.click(); URL.revokeObjectURL(url);
      window._tuUpdateStatus("✅ 配置已导出");
    });
    ...
    GM_registerMenuCommand('🗑️ 清除翻译缓存', () => { clearCache(); window._tuUpdateStatus('缓存已清空'); });

    cacheSaveTimer = setInterval(saveCache, CACHE_SAVE_INTERVAL);
    addEvent(window, 'beforeunload', saveCache);

    initMutationObserver();
    startMicrosoftMonitor();

    if (autoMode && !isPageInTargetLang() && displayMode !== 'original') {
      queueMicrotask(async () => { window._tuUpdateStatus(displayMode === 'bilingual' ? '双语翻译中...' : '自动翻译中...'); scanAndObserve(document.body); });
    }
  }

  function updateStatus(msg) { if (window._tuUpdateStatus) window._tuUpdateStatus(msg); }
