...
(async () => {
  ...
  function saveCache(immediate) {
    ...
    const doSave = () => {
      try {
        GM_setValue('translationCache', JSON.stringify(Object.fromEntries(cache)));
        cacheModified = false;
      } catch (e) {
    ...
    for (let i = 0; i < texts.length; i += AI_BATCH_SIZE) {
      ...
      try {
        ...
        for (let k = 0; k < parsed.length; k++) {
          if (!parsed[k]) {
            try { parsed[k] = await aiSingleTranslate(batch[k], toLang, apiKey, model, apiUrl, engineName); }
            catch (_) { parsed[k] = null; }
          }
        }
  ...
  const SCRIPT_PATTERNS = {
    latin: /^[\p{Script=Latin}\s\d\p{P}\p{S}]+$/u,
    cyrillic: /^[\p{Script=Cyrillic}\s\d\p{P}\p{S}]+$/u,
    arabic: /^[\p{Script=Arabic}\s\d\p{P}\p{S}]+$/u,
    greek: /^[\p{Script=Greek}\s\d\p{P}\p{S}]+$/u,
    hebrew: /^[\p{Script=Hebrew}\s\d\p{P}\p{S}]+$/u,
    devanagari: /^[\p{Script=Devanagari}\s\d\p{P}\p{S}]+$/u,
    thai: /^[\p{Script=Thai}\s\d\p{P}\p{S}]+$/u,
    lao: /^[\p{Script=Lao}\s\d\p{P}\p{S}]+$/u,
    khmer: /^[\p{Script=Khmer}\s\d\p{P}\p{S}]+$/u,
    myanmar: /^[\p{Script=Myanmar}\s\d\p{P}\p{S}]+$/u,
    bengali: /^[\p{Script=Bengali}\s\d\p{P}\p{S}]+$/u,
    tamil: /^[\p{Script=Tamil}\s\d\p{P}\p{S}]+$/u,
    telugu: /^[\p{Script=Telugu}\s\d\p{P}\p{S}]+$/u,
    kannada: /^[\p{Script=Kannada}\s\d\p{P}\p{S}]+$/u,
    malayalam: /^[\p{Script=Malayalam}\s\d\p{P}\p{S}]+$/u,
    gujarati: /^[\p{Script=Gujarati}\s\d\p{P}\p{S}]+$/u,
    gurmukhi: /^[\p{Script=Gurmukhi}\s\d\p{P}\p{S}]+$/u,
    sinhala: /^[\p{Script=Sinhala}\s\d\p{P}\p{S}]+$/u,
    georgian: /^[\p{Script=Georgian}\s\d\p{P}\p{S}]+$/u,
    armenian: /^[\p{Script=Armenian}\s\d\p{P}\p{S}]+$/u
  };
  ...
  const LANG_SCRIPT = {
    ru: 'cyrillic', uk: 'cyrillic', bg: 'cyrillic', sr: 'cyrillic', be: 'cyrillic', mk: 'cyrillic', kk: 'cyrillic', ky: 'cyrillic', mn: 'cyrillic', tg: 'cyrillic', tt: 'cyrillic',
    el: 'greek', he: 'hebrew',
    ar: 'arabic', fa: 'arabic', ur: 'arabic', ps: 'arabic', ckb: 'arabic', sd: 'arabic', ug: 'arabic',
    hi: 'devanagari', mr: 'devanagari', ne: 'devanagari', sa: 'devanagari', mai: 'devanagari', bho: 'devanagari',
    th: 'thai', lo: 'lao', km: 'khmer', my: 'myanmar',
    bn: 'bengali', as: 'bengali', ta: 'tamil', te: 'telugu', kn: 'kannada', ml: 'malayalam',
    gu: 'gujarati', pa: 'gurmukhi', si: 'sinhala', ka: 'georgian', hy: 'armenian'
  };
  ...
  function getLangRegex(lang) {
    ...
    if (!re) {
      const script = LANG_SCRIPT[lang] || (LATIN_LANGS.has(lang) ? 'latin' : null);
      if (script) re = SCRIPT_PATTERNS[script];
    }
