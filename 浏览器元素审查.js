...
(function() {
    ...
    const copyElement = (el) => {
        ...
        clone.classList.remove('mb-inspect-hl');
        if (clone.classList.length === 0) clone.removeAttribute('class');
        api.setClipboard(clone.outerHTML);
    };
    ...
    api.addStyle(`
        ...
        #mb-debug-panel {
            position: fixed; left: 2%; bottom: 0; width: 96%; height: 55%;
            ...
            border: 1px solid var(--mb-border); border-radius: 10px 10px 0 0;
            transition: none; color: var(--mb-text);
            overflow: hidden;
        }
        ...
        #mb-resize-handle {
            ...
            cursor: nesw-resize; z-index: 20; touch-action: none;
        ...
        .node-row { padding: 4px 4px; cursor: pointer; white-space: pre-wrap; word-break: break-all; display: flex; color: var(--mb-text); font-size: 13px; }
    ...
    panel.innerHTML = `
        ...
        <div id="mb-resize-handle" title="拖动缩放">
            <svg width="16" height="16" viewBox="0 0 16 16">
                <path d="M15 1 L15 6 M15 1 L10 1 M15 1 L8 8" stroke="#ff4757" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </div>
    `;
    ...
    function applyPanelHeight() {
        const h = panelDefaultHeight();
        panel.style.height = h;
        document.body.style.paddingBottom = h.replace('%', 'vh');
    }
    // 文字编辑时把面板收到只剩标题栏，方便看到并编辑网页
    function shrinkPanelForEdit() {
    ...
    (function initLogHook() {
        ...
        for (const key in levels) {
            ...
            console[key] = function(...args) {
                ...
                // 正在监听或错误级别时直接记录，无需解析调用栈
                if (isLogging || key === 'error') {
                    addLog(args, levels[key]);
                    return;
                }
                
                const stack = new Error().stack || '';
                if (stack.includes('eval') || stack.includes('anonymous')) {
                    addLog(args, levels[key]);
                }
            };
        }
    ...
    function viaApiInject(rule) {
        if (!rule) return;
        let vHost = location.hostname;
        let filter = rule.trim();
        if (filter.includes('##')) {
            const parts = filter.split('##');
            vHost = parts[0].trim() || location.host;
            filter = parts[1].trim();
        }
        if (typeof window.via === "object" && typeof window.via.record === "function") {
            try {
                window.via.record(vHost, filter);
                window.via.toast(`添加一条自定义规则${vHost}${filter}`);
                return;
            } catch (e) {
                console.error(e);
            }
        ...
        // 无论后续是否异常，都保证恢复原始 JSON.stringify
        try {
            via.cmd(517);
        } catch (e) {
            JSON.stringify = _s;
        ...
        setTimeout(() => {
            let restored = false;
            const restore = () => { if (!restored) { JSON.stringify = _s; restored = true; } };
            try {
            ...
            const updateUI = (isEditing = false) => {
                ...
                if (isEditing) {
                    ...
                    item.querySelector('.btn-copy').onclick = () => {
                        ...
                        else if (isVia) viaApiInject(currentRule);
        ...
        const updateSelection = () => {
            if (searchResults.length > 0) {
                ...
                if (treeContainer) {
                    ...
                    setTimeout(() => {
                        // 树节点实际使用 .node-row.selected 标记当前选中项
    ...
    function updateNodeActions() {
        ...
        const finishTextEdit = () => {
            ...
            applyPanelHeight(); startPicking(); clearAllHighlights(); renderDOM(); highlight(currentTarget); updateNodeActions();
        };

        btnSearch.onclick = (e) => { e.stopPropagation(); actionsBar.setAttribute('data-mode', 'search'); updateNodeActions(); };
        
        // 复制元素事件绑定（复用 copyElement）
        btnCopyNode.onclick = (e) => {
            e.stopPropagation();
        ...
        btnUndo.onclick = (e) => {
            ...
            currentTarget = restoredNode; applyPanelHeight(); startPicking(); renderDOM(); highlight(currentTarget); updateNodeActions();
        };
    }
    ...
    shadow.getElementById('mb-btn-copy-html').onclick = () => copyElement(currentTarget);
    ...
    const handler = (e) => {
        ...
        // 抬起/点击：判断是否为轻点（而非滑动）
        if (type === 'pointerup' || type === 'click' || type === 'touchend') {
    ...
    // 优先使用 Pointer Events，避免 pointer 与 mouse/touch 事件重复触发 handler
    const events = window.PointerEvent
        ? ['pointerdown', 'pointerup']
        : ['mousedown', 'touchstart', 'click', 'touchend'];
...
(function enableDragAndResize(){
    ...
    const wait = setInterval(()=>{
        ...
        // 拖动：用 transform 位移，避免触发重排
        let dStartX, dStartY, dBaseLeft, dBaseTop;
        // 缩放：改尺寸无法避免重排，用 rAF 节流缓解
        let rStartX, rStartY, rStartW, rStartH, rStartTop, rStartLeft;

        // rAF 节流：把一帧内的多次指针事件合并成一次 DOM 写入
        ...
        const schedule = (x, y) => {
            ...
            rafId = requestAnimationFrame(() => {
                rafId = null;
                if (isDragging) dragMove(lastX, lastY);
                else if (isResizing) resizeMove(lastX, lastY);
            });
        };
        ...
        const dragStart = (x, y) => {
            ...
            dStartX = x; dStartY = y;
            dBaseLeft = rect.left; dBaseTop = rect.top;
        };
        ...
        const dragMove = (x, y) => {
            ...
            // 约束在视口内：限制位移量而不是重新算 left/top
            const maxDx = (window.innerWidth - panel.offsetWidth) - dBaseLeft;
            const minDx = -dBaseLeft;
            const maxDy = (window.innerHeight - panel.offsetHeight) - dBaseTop;
            const minDy = -dBaseTop;
        ...
        const dragEnd = () => {
            if (!isDragging) return;
            isDragging = false;
            // 把 transform 位移固化回 top/left，清掉 transform，避免后续定位错乱
            const rect = panel.getBoundingClientRect();
        ...
        // ---------- 拖角缩放（右上角，锚定左下角：向上/向右扩展）----------
        const resizeStart = (x, y) => {
            isResizing = true;
            const rect = panel.getBoundingClientRect();
            panel.style.bottom = 'auto';
            panel.style.top = rect.top + 'px';
            ...
            panel.style.willChange = 'width, height, top';
            rStartX = x; rStartY = y;
        ...
        const resizeMove = (x, y) => {
            ...
            const dx = x - rStartX;
            const dy = y - rStartY;
            let newW = Math.max(MIN_W, Math.min(rStartW + dx, window.innerWidth - rStartLeft));
            let newH = Math.max(MIN_H, Math.min(rStartH - dy, rStartTop + rStartH));
            const newTop = rStartTop + rStartH - newH;
        ...
        // ---------- 绑定：缩放（右上角手柄），阻止冒泡避免触发拖动 ----------
        handle.addEventListener('mousedown', e => { e.stopPropagation(); e.preventDefault(); resizeStart(e.clientX, e.clientY); });
        handle.addEventListener('touchstart', e => { e.stopPropagation(); const t = e.touches[0]; resizeStart(t.clientX, t.clientY); }, {passive:true});

        // ---------- 全局 move / end（统一走 rAF 节流）----------
        ...
        document.addEventListener('touchmove', e => {
            if (!isDragging && !isResizing) return;
            const t = e.touches[0];
            if (isResizing && e.cancelable) e.preventDefault();
