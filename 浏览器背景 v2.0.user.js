...
` : `html::before { content:""!important; position:fixed!important; inset:0!important; z-index:-2147483647!important; pointer-events:none!important; background:transparent!important; opacity:0!important; }`;
    ...
    function hasOverlayKeyword(el) {
        ...
        return /overlay|backdrop|mask|modal|drawer|popup|dialog|sheet|menu|popover|sidebar|side-nav|side-panel|nav-panel|offcanvas|slide-panel|flyout|panel/.test(txt);
    }
    ...
    function findLikelyOverlays() {
        ...
        for (const el of document.body.querySelectorAll('*')) {
            ...
            const highZ = z >= 40;
            const mediumZ = z >= 10;  // 降低 z-index 要求
            const bigEnough = rect.width >= vw * 0.65 && rect.height >= vh * 0.65;
            const mediumLarge = rect.width >= vw * 0.35 && rect.height >= vh * 0.35;
            const smallLarge = rect.width >= vw * 0.2 && rect.height >= vh * 0.5;  // 侧边栏通常较窄但高
            const keyword = hasOverlayKeyword(el);
            const overlayBg = hasOverlayBg(style);

            // 原有检测逻辑
            if ((isFixed && highZ && bigEnough && (overlayBg || keyword)) ||
                ((isFixed || isAbsolute) && highZ && mediumLarge && (keyword || overlayBg)) ||
                (isFixed && highZ && keyword && rect.width >= vw * 0.5 && rect.height >= vh * 0.5)) {
                overlays.push(el);
                continue;
            }

            // 新增：针对侧边栏/遮罩层的检测（z-index 要求降低，尺寸要求调整）
            if ((isFixed || isAbsolute) && mediumZ && keyword && smallLarge) {
                overlays.push(el);
            }
        }
