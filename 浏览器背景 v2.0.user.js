...
` : `html::before { content:""!important; position:fixed!important; inset:0!important; z-index:-2147483647!important; pointer-events:none!important; background:transparent!important; opacity:0!important; }`;
    ...
    function pickLocalImage(callback) {
        ...
        input.addEventListener('change', () => {
            ...
            reader.onload = function () {
                ...
                if (isWebP) {
                    console.log('[浏览器背景] WebP 格式，保留透明度直接使用');
    ...
    // 用 DeflateRaw 压缩单个文件（利用 Compression Streams API）
    ...
    // 小文件直接 Store（不压缩），避免压缩后反而更大
    async function compressSmart(data) {
        if (data.length < 128) return { compressed: data, method: 0 }; // Store
        try {
            const deflated = await deflateRaw(data);
            if (deflated.length < data.length) return { compressed: deflated, method: 8 }; // Deflate
            return { compressed: data, method: 0 }; // Store
        } catch (e) {
            console.warn('[浏览器背景] 压缩失败，使用 Store:', e.message);
            return { compressed: data, method: 0 };
        }
    }

    // 写 little-endian uint16/uint32 到 DataView
    ...
    async function buildZip(entries) {
        ...
        let totalSize = 0;
        processed.forEach(p => {
            totalSize += 30 + p.nameBytes.length + p.compressedSize; // local file header
        });
        const centralDirStart = totalSize;
        processed.forEach(p => {
            totalSize += 46 + p.nameBytes.length; // central directory
        });
        ...
        for (const p of processed) {
            ...
            writeU16(view, offset + 8, p.method);     // compression method
            writeU16(view, offset + 10, 0);           // mod time
            writeU16(view, offset + 12, 0);           // mod date
            writeU32(view, offset + 14, p.crc);       // crc32
            writeU32(view, offset + 18, p.compressedSize);   // compressed size
            writeU32(view, offset + 22, p.originalSize);     // uncompressed size
            writeU16(view, offset + 26, p.nameBytes.length); // name length
        ...
        for (let i = 0; i < processed.length; i++) {
            ...
            writeU16(view, offset + 10, p.method);     // compression method
            writeU16(view, offset + 12, 0);            // mod time
            writeU16(view, offset + 14, 0);            // mod date
            writeU32(view, offset + 16, p.crc);        // crc32
            writeU32(view, offset + 20, p.compressedSize);   // compressed size
            writeU32(view, offset + 24, p.originalSize);     // uncompressed size
            writeU16(view, offset + 28, p.nameBytes.length); // name length
            ...
            writeU32(view, offset + 42, localOffsets[i]); // local header offset
            u8.set(p.nameBytes, offset + 46);
            offset += 46 + p.nameBytes.length;
        }

        // 写 end of central directory
        writeU32(view, offset, 0x06054b50);
        writeU16(view, offset + 4, 0);                 // disk number
        writeU16(view, offset + 6, 0);                 // central dir disk
        writeU16(view, offset + 8, processed.length);  // entries on disk
        writeU16(view, offset + 10, processed.length);  // total entries
    ...
    // 解析 ZIP（只读取 uncompressed/Store 文件，Deflate 不做解压，仅提取 config.json）
    async function parseZipForImport(zipData) {
        // 先尝试用 DecompressionStream 解压 deflate
        ...
        while (pos + 30 <= zipData.length) {
            ...
            try {
                ...
                console.warn('[浏览器背景] 解压文件失败:', name, e.message);
                pos = dataStart + compSize;
                continue;
            }
    ...
    function guessMimeFromUrl(url) {
        const ext = (url.match(/\.(\w{3,4})(?:[?#]|$)/i) || [])[1];
        if (!ext) return 'image/jpeg';
        const map = { jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp', svg: 'image/svg+xml', bmp: 'image/bmp', avif: 'image/avif' };
    ...
    function fetchFallback(url, resolve, reject) {
        ...
        fetch(url, {
            ...
            return resp.arrayBuffer().then(ab => {
                ...
                const mime = contentType.split(';')[0].trim() || guessMimeFromUrl(url);
                resolve({ data: new Uint8Array(ab), mime });
            });
        }).catch(err => {
            clearTimeout(timer);
            console.error('[浏览器背景] 图片下载失败 (' + url + '):', err.message || err);
        ...
        return new Promise((resolve, reject) => {
            if (typeof GM_xmlhttpRequest === 'function') {
                ...
                const timer = setTimeout(() => {
                    if (!settled) {
                        settled = true;
                        console.warn('[浏览器背景] GM_xmlhttpRequest 超时，回退到 fetch:', url);
                ...
                try {
                            ...
                            if (res.status >= 200 && res.status < 300) {
                                try {
                                    ...
                                    if (respData instanceof ArrayBuffer) {
                                        ...
                                        const mime = mimeMatch ? mimeMatch[1].trim() : guessMimeFromUrl(url);
                                        resolve({ data: new Uint8Array(respData), mime });
                                    } else if (respData && typeof respData.arrayBuffer === 'function') {
                                        const headerStr = String(res.responseHeaders || '');
                                        const mimeMatch = headerStr.match(/content-type:\s*([^\r\n;]+)/i);
                                        const mime = mimeMatch ? mimeMatch[1].trim() : guessMimeFromUrl(url);
                                        respData.arrayBuffer().then(ab => {
                                            resolve({ data: new Uint8Array(ab), mime });
                                        }).catch(err => {
                                            console.warn('[浏览器背景] Blob 转换失败，回退 fetch:', err);
                                            fetchFallback(url, resolve, reject);
                                        });
                                    } else {
                                        console.warn('[浏览器背景] GM 响应类型异常，回退 fetch');
                                        fetchFallback(url, resolve, reject);
                                    }
                                } catch (e) {
                                    console.warn('[浏览器背景] GM 处理响应出错，回退 fetch:', e);
                                    fetchFallback(url, resolve, reject);
                                }
                            } else {
                                console.warn('[浏览器背景] GM HTTP ' + res.status + '，回退 fetch:', url);
                                fetchFallback(url, resolve, reject);
                            }
                        },
                        onerror(err) {
                            ...
                            clearTimeout(timer);
                            settled = true;
                            console.warn('[浏览器背景] GM 请求出错，回退 fetch:', err);
                            fetchFallback(url, resolve, reject);
                        },
                        ontimeout() {
                            if (settled) return;
                            clearTimeout(timer);
                            settled = true;
                            console.warn('[浏览器背景] GM 请求超时，回退 fetch:', url);
                            fetchFallback(url, resolve, reject);
                        }
                    });
                } catch (e) {
                    if (!settled) {
                        clearTimeout(timer);
                        settled = true;
                        console.warn('[浏览器背景] GM 调用异常，回退 fetch:', e);
                        fetchFallback(url, resolve, reject);
                    }
                }
            } else {
        ...
        try {
            ...
            if (isDataImageUrl(globalUrl)) {
                ...
                try {
                    ...
                    console.log('[浏览器背景] 全局远程图片下载成功:', Math.round(data.length / 1024) + 'KB');
                } catch (e) {
                    console.warn('[浏览器背景] 下载全局图片失败:', e.message);
            ...
            if (isDataImageUrl(globalUrl) && globalImageFile) {
                imageEntries.push({ name: 'images/' + globalImageFile, data: dataUrlToUint8Array(globalUrl) });
            }
            ...
            for (const host in siteConfigMap) {
                ...
                if (isDataImageUrl(url)) {
                    ...
                    try {
                        ...
                        console.log('[浏览器背景] 站点图片下载成功 (' + host + '):', Math.round(data.length / 1024) + 'KB');
                    } catch (e) {
                        failedRemoteCount++;
                        console.warn('[浏览器背景] 下载站点图片失败 (' + host + '):', e.message);
                    }
                }
            ...
            // 用原生 Compression Streams API 打包 ZIP
    ...
    async function importAllConfigZip() {
        ...
        input.addEventListener('change', async () => {
            ...
            try {
                ...
                const gLocalDataUrl = (gLocalFile && imgData[gLocalFile]) ? imgData[gLocalFile] : '';
                if (gRemoteUrl) {
                    configData.global.url = gRemoteUrl;
                } else if (gLocalDataUrl) {
                    configData.global.url = gLocalDataUrl;
                } else {
                    configData.global.url = DEFAULTS.url;
                }
                ...
                if (configData.siteConfigMap) {
                    for (const host in configData.siteConfigMap) {
                        ...
                        const sLocalDataUrl = (sLocalFile && imgData[sLocalFile]) ? imgData[sLocalFile] : '';

                        if (sRemoteUrl) {
                            sc.url = sRemoteUrl;
                        } else if (sLocalDataUrl) {
                            sc.url = sLocalDataUrl;
                        } else {
                            delete configData.siteConfigMap[host].url;
                        }
                        delete sc.remoteUrl;
                    }
                }
    ...
    function applyImportedConfig(data) {
        ...
        configCache = null;
        const host = getHost();
        const gInput = document.getElementById('vie-v73-global-image-url');
        const sInput = document.getElementById('vie-v73-site-image-url');
        if (gInput) {
            const gUrl = getValue('Vie背景图片', DEFAULTS.url);
            gInput.value = isDataImageUrl(gUrl) ? '' : gUrl;
        }
        if (sInput) {
            const importedSiteCfg = getSiteConfig(host);
            if (importedSiteCfg && importedSiteCfg.url && !isDataImageUrl(importedSiteCfg.url)) {
...
`;
    ...
    function createFloat() {
        ...
        panelEl.id = 'vie-bg-panel-v73';

        // ===== 第一部分：切换按钮 =====
        box.innerHTML = '<div id="vie-bg-toggle-v73" title="点击展开/收起；拖动移动">𖣐</div>';
    ...
    <div class="btns" style="margin-top:6px!important;">
        ...
        advPanel.id = 'vie-v73-advanced-panel';
        ...
        box.querySelector('#vie-v73-save-site').addEventListener('click', (e) => {
            ...
            const h = getHost();
            configCache = null;
            const existingSiteCfg = getSiteConfig(h) || {};
            setSiteConfig(h, Object.assign({}, existingSiteCfg, {
                url: v.siteImageUrl || existingSiteCfg.url,
                opacity: v.opacity,
                blur: v.blur,
        ...
        // ===== 导入导出按钮事件 =====
        box.querySelector('#vie-v73-export').addEventListener('click', (e) => {
            e.stopPropagation();
            exportAllConfigZip();
        });

        box.querySelector('#vie-v73-import').addEventListener('click', (e) => {
            e.stopPropagation();
            importAllConfigZip();
        });
