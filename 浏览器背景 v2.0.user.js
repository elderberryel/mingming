...
` : `html::before { content:""!important; position:fixed!important; inset:0!important; z-index:-2147483647!important; pointer-events:none!important; background:transparent!important; opacity:0!important; }`;
    ...
    function fetchImageAsUint8(url) {
        return new Promise((resolve, reject) => {
            // 方案1：优先使用 GM_xmlhttpRequest（跨域能力最强）
            if (typeof GM_xmlhttpRequest === 'function') {
                ...
                // 方案2：无 GM_xmlhttpRequest 时直接用 fetch
                fetchFallback(url, resolve, reject);
            }
        });
    }
    ...
    function fetchFallback(url, resolve, reject) {
        ...
        fetch(url, {
            ...
            clearTimeout(timer);
            // fetch 也失败，最后尝试 no-cors 模式（虽然拿不到数据，但至少不会报错）
    ...
    async function exportAllConfigZip() {
        try {
            ...
            if (globalRemoteData && globalImageFile) {
                zipFiles['images/' + globalImageFile] = globalRemoteData;
            }
