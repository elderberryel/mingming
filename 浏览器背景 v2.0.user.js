        ...
        img.onload = function () {
            ...
            if (isPng) {
                ...
                if (!testWebP.startsWith('data:image/webp')) {
                    ...
                    if (result.length > targetSize && width > 400 && height > 400) {
                        const scaleRatio = Math.sqrt(targetSize / result.length) * 0.9;
                        width = Math.max(200, Math.round(width * scaleRatio));
                        height = Math.max(200, Math.round(height * scaleRatio));
                        canvas.width = width;
                        canvas.height = height;
                        ctx.drawImage(img, 0, 0, width, height);
                ...
                if (result.length > targetSize * 1.2 && width > 400 && height > 400) {
                    const scaleRatio = Math.sqrt(targetSize / result.length) * 0.9;
                    width = Math.max(200, Math.round(width * scaleRatio));
                    height = Math.max(200, Math.round(height * scaleRatio));
                    canvas.width = width;
                ...
                console.log(`[浏览器背景] PNG→WebP: 质量 ${Math.round(quality * 100)}%, ${Math.round(result.length / 1024)}KB`);
                callback(result);
                return;
            }
